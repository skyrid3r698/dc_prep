...
function check {
    ...
    if ($RDSCollection -ne $null) {if ((Get-RDSessionCollectionConfiguration -ConnectionBroker "$brokerInstalled.$domainnamelong" -CollectionName $RDSCollection -Connection).DisconnectedSessionLimitMin -gt 1) {Write-Host $(Get-Date)"[INFO] RDSessionCollection is configured properly"} else {Write-Host $(Get-Date)"[INFO] RDSessionCollection DisconnectedSessionLimit is not set. It is recommended to set one."}} else {Write-Host $(Get-Date)"[WARNING] No RDSessionCollection is configured. It is recommended to configure a Collection even with one Terminalserver" -ForegroundColor Yellow}
    #summary
